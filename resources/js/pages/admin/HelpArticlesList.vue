<!-- Centre d'aide — gestion : page unique avec onglets Articles | Catégories. -->
<template>
  <AdminLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between">
        <h1 dusk="help-admin-title" class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('help.admin.title') }}</h1>
        <div class="flex items-center gap-2">
          <router-link
            v-if="tab === 'articles' && canCreateHelpArticles"
          ...
          <button
            v-if="tab === 'categories' && canManageHelpCategories"
            @click="openCreateCategory"
            class="rounded-3 bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"
          >
            {{ $t('help.admin.new_category') }}
          </button>
        </div>
      </div>
      ...
      <nav class="flex gap-1 border-b border-gray-200 dark:border-gray-800">
        <button
          ...
          :key="tabItem.key"
          :dusk="`help-admin-tab-${tabItem.key}`"
          ...
          <table class="min-w-full text-sm">
            <thead class="border-b border-gray-200 text-left text-xs uppercase text-gray-400 dark:border-gray-800">
              <tr>
                ...
                <th class="px-4 py-3">{{ $t('help.admin.cat_slug') }}</th>
                <th class="px-4 py-3">{{ $t('help.admin.cat_position') }}</th>
                <th class="px-4 py-3">{{ $t('help.admin.col_status') }}</th>
                <th class="px-4 py-3">{{ $t('help.admin.cat_articles') }}</th>
                <th class="px-4 py-3 text-right">{{ $t('help.admin.col_actions') }}</th>
              </tr>
            </thead>
            <tbody ref="catRowsRef" class="divide-y divide-gray-100 dark:divide-gray-800">
              <tr v-for="cat in categories" :key="cat.id" class="stagger-item">
                <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">
                  <span class="mr-2"><i :class="['fas', cat.icon || 'fa-book', 'text-gray-400']"></i></span>
                  {{ cat.nom_fr }}
                </td>
                <td class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ cat.slug }}</td>
                <td class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ cat.position }}</td>
                <td class="px-4 py-3">
                  <span
                    class="rounded-full px-2 py-0.5 text-xs font-medium"
                    :class="cat.published_at
                      ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400'
                      : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'"
                  >
                    {{ cat.published_at ? $t('help.admin.status_published') : $t('help.admin.status_draft') }}
                  </span>
                </td>
                <td class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ cat.articles_count ?? 0 }}</td>
                <td class="px-4 py-3">
                  <div class="flex items-center justify-end gap-2">
                    <button
      ...
      <!-- Modale création / édition de catégorie (partagée) -->
      <HelpCategoryModal
...
const { staggerRef: catRowsRef, applyStagger: applyCatStagger } = useStagger(40)
...
const tabs = computed(() => {
  const list = [{ key: 'articles', label: t('help.admin.tab_articles') }]
  if (canManageHelpCategories.value) {
    list.push({ key: 'categories', label: t('help.admin.tab_categories') })
  }
  return list
})
...
const removeCategory = async (cat) => {
  if (!window.confirm(t('help.admin.confirm_delete_category'))) return
  await api.delete(`/admin/help/categories/${cat.id}`)
  fetchCategories()
}

// Charge les catégories à la première ouverture de l'onglet (lazy).
const ensureCategories = () => {
  if (!categoriesLoaded.value) fetchCategories()
}

// Recharge les catégories (lazy) quand on bascule sur leur onglet.
watch(tab, (value) => {
  if (value === 'categories') ensureCategories()
})
