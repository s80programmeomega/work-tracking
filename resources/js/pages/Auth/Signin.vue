    ...
    <FullScreenLayout>
        <div class="relative p-6 bg-white z-1 dark:bg-gray-900 sm:p-0">
            <div class="relative flex flex-col justify-center w-full h-screen lg:flex-row dark:bg-gray-900">
                            ...
                            <div>
                                ...
                                <form @submit.prevent="handleSubmit">
                                    <div class="space-y-5">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <label for="keepLoggedIn"
                                                    ...
                                                    <div class="relative">
                                                        <input v-model="form.remember" type="checkbox" id="keepLoggedIn"
                                        ...
                                        <div v-if="registrationSuccess" class="rounded-3 bg-success-50 p-4 dark:bg-success-500/10">
                ...
                <!-- Colonne droite - Illustration avec nouveau branding -->
...
const getTranslatedError = (error: string): string => {
    if (error.includes('The email field is required')) {
        return t('validation.required', { attribute: t('auth.email').toLowerCase() });
    }
    if (error.includes('The password field is required')) {
        return t('validation.required', { attribute: t('auth.password').toLowerCase() });
    }
    if (error.includes('The email must be a valid email address')) {
        return t('validation.email');
    }
    if (error.includes('The password must be at least')) {
        return t('validation.min.string', { attribute: t('auth.password').toLowerCase(), min: 8 });
    }
    return error;
};
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (error.response?.status === 401) {
            ...
            authStore.error = t('auth.login_failed');
        }
    }
};
