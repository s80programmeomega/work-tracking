        ...
        <div v-for="group in tasksByActivite" :key="group.activite.id"
          ...
          <div class="p-4 grid grid-cols-3 gap-3">
            <div v-for="col in kanbanColumns" :key="col.statut" class="bg-gray-50 dark:bg-gray-900 rounded-3 p-3">
              ...
              <div class="space-y-2">
                <TacheCardPersonal v-for="tache in group.taches.filter(t => getMyStatus(t) === col.statut)"
                  :key="tache.id" :tache="tache" compact @view="handleViewTask" @move="handleMoveMyCard"
                  @submit-result="handleSubmitResult" @edit="handleEditTask" />
              </div>
            </div>
          </div>
        </div>
...
// ✅ NOUVEAU : Gestion de l'édition (pour responsables)
async function handleEditTask(tache) {
  try {
    // Charger la tâche complète
    const { data } = await api.get(`/taches/${tache.id}`)
    currentTache.value = data.data
...
// ✅ NOUVEAU : Après sauvegarde de l'édition
