...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div
        ...
        <div class="w-full bg-blue-200 dark:bg-blue-900 rounded-full h-2 mt-3">
          <div class="h-2 rounded-full transition-all duration-300" :class="getProgressColor(tache.taux_realisation)"
            :style="{ width: `${tache.taux_realisation}%` }"></div>
        </div>
      </div>
      ...
      <div
        ...
        <div v-if="tache.echeance">
          <p class="text-lg font-bold"
            :class="tache.is_overdue ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'">
        ...
        <div class="space-y-4">
          ...
          <div v-if="tache.echeance" class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
              ...
              <svg class="w-5 h-5"
                :class="tache.is_overdue ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'"
          ...
          <div v-if="tache.week_number" class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-sm font-medium text-gray-900 dark:text-white">S{{ tache.week_number }} - {{ tache.year }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 rounded-lg border-2 transition-all" :class="tache.validation.n1_validated_at
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              ...
              <p v-if="tache.validation.n1_validated_at"
              ...
              <p v-if="tache.validation.n1_commentaire"
                class="text-sm text-gray-700 dark:text-gray-300 mt-2 p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 italic">
        ...
        <div v-if="tache.validation.n2_required" class="p-4 rounded-lg border-2 transition-all" :class="tache.validation.n2_validated_at
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              ...
              <p v-if="tache.validation.n2_validated_at"
                class="text-sm text-gray-600 dark:text-gray-400 mt-1 flex items-center gap-2">
              ...
              <p v-if="tache.validation.n2_commentaire"
                class="text-sm text-gray-700 dark:text-gray-300 mt-2 p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 italic">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- Fichiers attachés -->
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <TacheAttachmentsSection :tache-id="tache.id" :attachments="tache.attachments || []"
          :can-edit="tache.permissions?.can_edit" @updated="$emit('refresh')" />
      </div>

      <!-- Liens externes -->
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <TacheExternalLinksSection :tache-id="tache.id" :links="tache.external_links || []"
          :can-edit="tache.permissions?.can_edit" @updated="$emit('refresh')" />
      </div>
    </div>
