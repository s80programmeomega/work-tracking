  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
        ...
        <div class="flex items-end gap-2">
          <span class="text-3xl font-bold text-gray-900 dark:text-white">{{ tache.taux_realisation }}%</span>
        </div>
      ...
      <div class="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl p-4 border border-purple-200 dark:border-purple-800">
        ...
        <div class="flex items-end gap-2">
          ...
          <span class="text-sm text-gray-600 dark:text-gray-400 mb-1">
            / {{ tache.estimated_hours || 0 }}h
          </span>
        </div>
        <p v-if="tache.estimated_hours && tache.actual_hours" class="text-xs mt-1" :class="getVarianceClass(tache.actual_hours - tache.estimated_hours)">
      ...
      <div class="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800">
        ...
        <div v-if="tache.echeance">
          <p class="text-lg font-bold" :class="tache.is_overdue ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'">
            {{ formatDate(tache.echeance) }}
          </p>
          <p v-if="tache.is_overdue" class="text-xs text-red-600 dark:text-red-400 mt-1 flex items-center gap-1">
          ...
          <p v-else class="text-xs text-gray-600 dark:text-gray-400 mt-1">
            {{ getTimeRemaining(tache.echeance) }}
          </p>
        </div>
        <p v-else class="text-sm text-gray-500 dark:text-gray-400">Non définie</p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div v-if="tache.description" class="prose prose-sm dark:prose-invert max-w-none">
        ...
        <div v-else class="text-center py-8">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Aucune description</p>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div v-if="tache.objectif" class="prose prose-sm dark:prose-invert max-w-none">
    ...
    <div v-if="tache.indicateurs_resultats" class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-800 p-6">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
        <svg class="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-4">
          ...
          <div v-if="tache.echeance" class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                 :class="tache.is_overdue ? 'bg-red-100 dark:bg-red-900/30' : 'bg-amber-100 dark:bg-amber-900/30'">
              <svg class="w-5 h-5" :class="tache.is_overdue ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400">Échéance</p>
              <p class="text-sm font-medium" :class="tache.is_overdue ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'">
                {{ formatDate(tache.echeance) }}
              </p>
            </div>
          </div>
    ...
    <!-- Validation Status -->
    <div v-if="tache.validation" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
        ...
        <div class="p-4 rounded-lg border-2 transition-all" :class="tache.validation.n1_validated_at 
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              ...
              <p v-if="!tache.validation.n1_validated_at && tache.validation.n1_required" 
                 class="text-sm text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
          ...
          <div class="flex items-start gap-4">
            <div class="flex-shrink-0">
              <div v-if="tache.validation.n2_validated_at" 
                   class="w-12 h-12 bg-purple-500 rounded-full flex items-center justify-center shadow-lg">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
              </div>
            ...
            <div class="flex-1">
              ...
              <p v-if="!tache.validation.n2_validated_at && tache.validation.n1_validated_at" 
                 class="text-sm text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- Fichiers attachés -->
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <TacheAttachmentsSection
          :tache-id="tache.id"
          :attachments="tache.attachments || []"
          :can-edit="tache.permissions?.can_edit"
      ...
      <!-- Liens externes -->
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <TacheExternalLinksSection
          :tache-id="tache.id"
          :links="tache.external_links || []"
          :can-edit="tache.permissions?.can_edit"
          @updated="$emit('refresh')"
        />
      </div>
    ...
    <!-- Commentaire -->
    <div v-if="tache.commentaire" class="bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800 p-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
          ...
          <p class="text-sm text-gray-900 dark:text-white">{{ formatDate(tache.updated_at) }}</p>
        </div>
        <div class="p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
...
import TacheAttachmentsSection from './TacheAttachmentsSection.vue'
import TacheExternalLinksSection from './TacheExternalLinksSection.vue'
