...
| 13b | Admin audit log: migration, model, service, frontend page | new files | ✅ Done | `AdminAuditLog.vue` + router `/admin/audit-log` + sidebar link + i18n. Backend (migration, model, service, controller endpoints) was already done. `DirecteurAuditLog.vue` already existed. |
