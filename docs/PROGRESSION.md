...
| — | Phase 1 — Visibility Full Replacement (Steps 1.1–1.6) | `feature/visibility-teams-chat` | 🔄 | 2026-06-27 | — | Dropped `visibility` columns from projets/taches/documents/teams (4 additive migrations with `Schema::hasColumn` guards). Added `PROJETS_VIEW_ALL` permission (manager+). Rewired `ProjetPolicy`, `DocumentPolicy`, `DocumentAccessResolver`, `TacheService`, `Document::scopeAccessibleBy()`, `Document::canBeViewedBy()` to role-based access only. Removed `default_project_visibility` from Workspace model/controller/factory/seeder and all frontend (Create, Edit, Settings, fr.json). Cleaned all visibility UI from 15+ Vue components (DocumentCard, DocumentUpload, DocumentUploadModal, DocumentEditModal, DocumentViewerModal, DocumentList, DocumentUploader, TacheTable, TacheForm, TacheDetailsTab, DetailedTaskView, Teams, Teams/Show, ProjetDetail). Pint clean, Larastan 0 errors. Tests (ProjetAccessTest, TachePolicyTest, DocumentAccessTest) deferred to end of plan. |
