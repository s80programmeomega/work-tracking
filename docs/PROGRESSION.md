...
| — | Phase 11 — Dashboard Accuracy + Profile Page Fixes | `feature/phase11{a-e}-*` | 🔄 | 2026-06-10 | — | 5 sub-branches (11A-11E). **11A done** (2026-06-10): real period-over-period `change`/`trend` for dashboard stats (`projets_actifs`/`taux_completion`/`taches_en_retard`), Kanban aligned to `ProjetStatus` (`active`/`completed`, `getRecentProjects` includes both), `common.completed` i18n key added, debug console.log removed, admin `tasks.overdue` switched to `Tache::overdue()`. 5 new tests, Pint/Larastan/build green. **11B done** (2026-06-10): functional notification sound (`useNotificationSound.js` + `public/sounds/notification.ogg`, wired into `App.vue`'s live-notification handler, gated on `notificationSettings.notificationSounds`); Preferences tab reduced to Timezone only (Theme/Language/Date Format/Display Density/Auto-Save removed, unused i18n keys cleaned up); dead `/settings` navbar link + `user_menu.account_settings` i18n key removed. No JS test runner — manual steps documented. Pint/Larastan/build green. **11C done** (2026-06-10): `GET /api/users/sessions` endpoint (scoped to current user, `is_current` flag); `authStore.fetchSessions()` + `logoutAllDevices()`; `SessionSettings.vue` rewritten with real session list + stagger + Tailwind; 5 PHPUnit tests; Pint/Larastan/build green. **11D done** (2026-06-10): migration adds `draft_body_fr`/`draft_body_en`/`draft_saved_at` to `help_articles`; `PATCH /admin/help/articles/{article}/draft` endpoint (`updateQuietly()` bypasses Purifier hook on live columns); `HelpArticleForm.vue` — debounced 5s auto-save + "Draft saved at HH:MM" indicator + "Restore/Discard" banner when draft is newer than last save; `SaveHelpArticleDraftRequest`; 7 PHPUnit tests; Pint/Larastan/build green. **11E pending**: profile activity tab + super-admin/workspace-owner per-user activity views + new workspace Users page. Plan + tracker in `docs/phase11-dashboard-profile/`. |
