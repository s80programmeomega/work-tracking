...
| — | Phase 6 — Global search (Typesense + Scout) | `feature/phase6-search` | 🔄 | 2026-06-04 | — | laravel/scout + typesense/typesense-php installed, config/scout.php with 5 model schemas, SCOUT_DRIVER=collection for tests, search.global permission (Guide 4+15 full flow), Projet/Activite/Tache/Document/User Searchable, SearchController GET /api/search (->query() workspace scope), command-palette SearchBar.vue (Cmd+K, gated), 8 i18n keys, 8 PHPUnit tests green. Awaiting commit + merge. |
