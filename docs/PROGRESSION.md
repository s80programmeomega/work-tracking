...
| — | Phase 5 — Chat real-time, @mentions, typing, unread badge, attachments | `feature/phase5-chat` | ✅ | 2026-06-04 | 2026-06-04 | 4 broadcast events (ShouldBroadcastNow), team.{teamId} channel auth, ChatMentionNotification, NotificationService extended, TeamMessageResource, +5 controller methods, +7 routes, migration last_read_at, useTeamMessages.js rewrite (Echo + optimistic + whisper), Teams/Show.vue full chat UI overhaul, AppSidebar unread badge. 12 PHPUnit + 3 Dusk tests. Merged into jonas 2026-06-04. |
| — | Phase 6 — Global search (Typesense + Scout) | `feature/phase6-search` | ✅ | 2026-06-04 | 2026-06-06 | Command palette + full `/search` page + topbar button; 3 role tiers (`search.global` + `search.scoped`); 8 Searchable models (incl. SousTache + user-scoped Notification); file content extraction; Typesense highlights; Excel export (manager+). **Security:** fixed Typesense `fromRaw` cross-workspace scoping leak (filter_by) + `exportSelected` IDOR. 717 tests. Merged into `jonas` 2026-06-06. |
| — | Phase 7 — Help Center (reader + author, no AI) | `feature/phase7-help-center` | 🚧 | 2026-06-06 | — | Bilingual categories/articles; Purifier sanitize-on-save (`help-article` allowlist) + plain-text extraction; MySQL FULLTEXT `scopeSearch` (boolean/prefix); `help_articles.read` (all roles) + `help_articles.manage` (owner/directeur + super_admin); `HelpController` (read) + `AdminHelpController` (CRUD/publish/images) + 4 Form Requests + 17 routes. Reader UI (`/help`, `/help/c/:slug`, `/help/a/:slug`) + author UI (Tiptap editor, list, form) + sidebar entry + fr/en i18n. 15 PHPUnit tests, Pint + Larastan clean, build green. Tiptap deps added (approved). Committed `92c482a` — **not merged, not pushed.** |
| — | Phase 11 — Dashboard Accuracy + Profile Page Fixes | `feature/phase11{a-e}-*` | ✅ | 2026-06-10 | 2026-06-10 | All 5 sub-branches complete. **11A**: real PoP stats, Kanban fix, debug log removed, admin overdue semantics. **11B**: notification sound, Preferences tab trimmed, dead navbar link removed. **11C**: real session list + revoke + stagger. **11D**: Tiptap draft auto-save + restore/discard banner. **11E** (2026-06-10): `ActivityLog.vue` wired to real API + stagger; `ActivityLogTab` `fixedCauserId` prop; "Activity" button in `AdminUsers.vue`; `WORKSPACES_VIEW_MEMBERS` permission (owner/manager); `GET /api/workspaces/{workspace}/users` endpoint; `WorkspaceUsers.vue` page + route + sidebar; 8 PHPUnit + 3 Dusk tests; `PERMISSIONS_MATRIX.md` updated. Plan + tracker in `docs/phase11-dashboard-profile/`. |
| — | Superadmin Platform-Only Scoping + Workspace Ban/Member Management + Temp Admin Lifecycle + Session Management Fixes + Admin Audit Log + Document Sharing Fixes | `feature/superadmin-scoping` | ✅ | 2026-06-25 | 2026-06-27 | **Scoping (Steps 1–17):** Remove `Gate::before()` bypass; named platform gates; all unscoped SA data paths closed; `ContextualPermissionGate` no bypass; `temporary_access` table for workspace grants; `admin:create-superadmin` + `admin:expire-accounts` commands; router SA redirect; all 6 permission composable bypasses removed; ChangePasswordModal wired; password reset flow fixed. **Ban/unban:** migration (`banned_at/by/reason`), `WorkspacePolicy::banMember`, `WorkspaceController::banMember/unbanMember`, `WorkspaceMemberBannedNotification`, 11 PHPUnit tests. **Workspace member UI:** `WorkspaceMembers.vue` (members + invitations tabs), sidebar "Gestion du workspace" section (isDirecteur-gated). **Temp admin:** `createTempAdmin` (brand-new User), `sendTempAdminCredentials`, `terminate`, `reactivateTempAdmin`; `workspace_role` field; auto-provision `workspace_members` on switch; custom permissions JSON; 15 PHPUnit + 6+8 Dusk tests. **Session management:** UA display per session (browser+OS parsing, mobile/desktop icon, DB::table UA write fix); reactive termination via `SessionRevoked`/`SessionsAllRevoked` ShouldBroadcastNow events + `useLiveNotifications` listeners → instant logout. **Document sharing:** workspace-scoped member search (`WorkspaceController::searchMembers`), `DocumentResource.workspace_id`, `sharedWithMe` cross-workspace fix (removed workspace filter), share link fixed to frontend URL, `DocumentPolicy` explicit permission checks for all 5 flags (`can_view`, `can_download`, `can_edit`, `can_delete`, `can_share`), `download` policy split from `view`. **Step 13b:** `AdminAuditLogTab.vue` + admin logs tab + directeur audit log view. **Tests:** 872 passed, 6 skipped, 0 failed. Merged into `jonas` 2026-06-27, pushed to both remotes. |
| — | Phase 1 — Visibility Full Replacement (Steps 1.1–1.6) | `feature/visibility-teams-chat` | ✅ | 2026-06-27 | 2026-06-28 | Dropped `visibility` columns from projets/taches/documents/teams (4 additive migrations with `Schema::hasColumn` guards). Added `PROJETS_VIEW_ALL` permission (manager+). Rewired `ProjetPolicy`, `DocumentPolicy`, `DocumentAccessResolver`, `TacheService`, `Document::scopeAccessibleBy()`, `Document::canBeViewedBy()` to role-based access only. Removed `default_project_visibility` from Workspace model/controller/factory/seeder and all frontend (Create, Edit, Settings, fr.json). Cleaned all visibility UI from 15+ Vue components. Pint clean, Larastan 0 errors. **Tests:** `ProjetAccessTest` (10 tests), `TachePolicyTest` (8 tests), `DocumentAccessTest` (8 tests) — all passing 2026-06-28. |
| — | Phase 2 — Team Integration (Steps 2.1–2.6) | `feature/visibility-teams-chat` | ✅ | 2026-06-27 | 2026-06-28 | `use_teams` toggle on Projet; `PROJETS_MANAGE_TEAMS` permission; `ProjetTeamController` (index/link/unlink/toggleUseTeams/candidates); `TeamLinkedToProject`/`TeamUnlinkedFromProject` events; `TeamProjectObserver` member sync; `TeamMemberAutoAddedNotification`; `ProjetDetail.vue` team panel; `ProjetForm.vue` use_teams field; `TeamResource`. **Tests:** `TeamProjectIntegrationTest` (14 tests) — all passing 2026-06-28. |
| — | Phase 3 — Workspace Chat (channels, messages, reactions, unread, pins, mentions) | `feature/visibility-teams-chat` | ✅ | 2026-06-27 | 2026-06-28 | `WorkspaceChannel`/`WorkspaceMessage`/`WorkspaceChannelRead`/`WorkspaceMessageReaction` models + migrations; `WorkspaceChatController`; `WorkspaceObserver` auto-creates global+responsibles channels; `WorkspaceMessagePolicy`; `WorkspaceMessageService`; `WorkspaceMessageMentionNotification`; broadcast events; `useWorkspaceMessages.js`; `WorkspaceChat.vue` page. **Tests:** `WorkspaceChatTest` (18 tests) — all passing 2026-06-28. |
| — | Chat Polish — Sound fix, Responsibles tab, notification audit, team chat parity | `feature/visibility-teams-chat` | ✅ | 2026-06-28 | 2026-06-28 | Sound guard (`initialLoadDone`), `getWorkspaceRole()` fix for responsibles tab; `App.vue` extended title extraction + `NotificationMenu.vue` live refresh; team chat: 50-color BUBBLE_PALETTE, emoji limit (max 3), sound watcher, photo upload with inline preview. Pint clean, Larastan 0 errors. |
| — | Pre-merge regression sweep (first full-suite run on this branch) | `feature/visibility-teams-chat` | ✅ | 2026-06-30 | 2026-06-30 | Found + fixed 5 failures: `ValidatorPendingListTest` (2, test setup bug — new `EVALUATIONS_VIEW_PENDING` gate from `3a0b29d` was correctly rejecting a validator not actually a member of the workspace under test); `StoreTeamMessageRequest` `content` wrongly made fully optional by Chat Polish's `attachments_json` change → 500 on empty payload, now conditionally required unless an attachment is present; `mentions` validation switched to `string`-only (FormData support) broke JSON API callers → fixed via `prepareForValidation()` normalizing string-or-array input. 912 passed, 6 skipped, 1 pre-existing unrelated flaky failure (`PlatformDashboardTest`, passes standalone). Pint clean, Larastan 0 errors. |
| — | Role label centralization + "Sous-tâche"→"Opération" rename + Profile CRUD sections | `feature/role-label-centralization` | 🔄 | 2026-06-30 | — | **A** done: `RoleLabel.php` + bilingual lang files; 6 Notification classes fixed (wrong vocab); `Permission.js` updated; `useWorkspace.js` cascade fix. **B** done: all French "Sous-tâche" display text → "Opération" in lang files + 5 Vue components; internal identifiers untouched. **C.1–C.2+C.4+C.5** done: 4 new tables/models/factories, Form Requests, controllers, routes, `profileView()` extended, 4 Vue section components (`readonly`+`initialData` dual-use), `UserProfile.vue` new tab, `UserProfileModal.vue` read-only, i18n, 19 tests passing. **C.3 (CV upload) not started** — touches shared `DocumentService`. Not committed. |
