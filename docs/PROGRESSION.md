...
| — | Superadmin Platform-Only Scoping + Workspace Ban/Member Management + Temp Admin Lifecycle + Session Management Fixes + Admin Audit Log | `feature/superadmin-scoping` | 🔄 | 2026-06-25 | — | **Scoping (Steps 1–17):** Remove `Gate::before()` bypass; named platform gates; all unscoped SA data paths closed; `ContextualPermissionGate` no bypass; `temporary_access` table for workspace grants; `admin:create-superadmin` + `admin:expire-accounts` commands; router SA redirect; all 6 permission composable bypasses removed; ChangePasswordModal wired; password reset flow fixed. **Ban/unban:** migration (`banned_at/by/reason`), `WorkspacePolicy::banMember`, `WorkspaceController::banMember/unbanMember`, `WorkspaceMemberBannedNotification`, 11 PHPUnit tests. **Workspace member UI:** `WorkspaceMembers.vue` (members + invitations tabs), sidebar "Gestion du workspace" section (isDirecteur-gated). **Temp admin:** `createTempAdmin` (brand-new User), `sendTempAdminCredentials`, `terminate`, `reactivateTempAdmin`; `workspace_role` field; auto-provision `workspace_members` on switch; custom permissions JSON; 15 PHPUnit + 6+8 Dusk tests. Committed `e8458f0`. **Session management fixes:** Guide 28; **Task B** inactivity timer (multiplier 1→60×1000, Infinity/"Jamais" sentinel, init restores saved setting, reactive countdown display); **Task C** session refresh (localStorage sync, expired-token guard, remember-me duration preservation, route moved to auth:sanctum); **Task A** multi-session (removed single-session lock from issueToken, logoutAll + revokeSession endpoints, SessionRevokedNotification, per-session revoke UI); **Task D** push notifications (NotificationSettings.vue rewritten to real API + useWebPush, toast replaces alert, browser-support/permission-denied warnings). 11 new PHPUnit tests (MultiSessionTest + TokenRefreshTest). **Step 13b:** `AdminAuditLog.vue` + router `/admin/audit-log` + sidebar link + i18n. 9 Dusk tests pass (InactivityTimer ×3, MultiSession ×3, NotificationSettings ×3), 14 screenshots captured. Pint + Larastan clean, build green. **Not yet committed, not yet pushed or merged into `jonas`.** |
