...
| Page | Status | Date | Findings |
|---|---|---|---|
| `pages/Auth/Signin.vue` | ✅ | 2026-05-30 | Cleaned `console.log('✅ CSRF Cookie test:', ...)` debug noise — kept the underlying `fetch('/sanctum/csrf-cookie')` call (necessary for Sanctum SPA stateful auth). authStore.login flow + error mapping otherwise clean. |
| `pages/Auth/Signup.vue` | ✅ | 2026-05-30 | Cleaned `console.log('🔄 Début de la tentative de connexion...')` debug (also a wrong-handler label — said "connexion" but it's register). Replaced English fallback string "Account created successfully! Please sign in." with French (Guide 19). |
| `pages/AcceptInvitation.vue` | ✅ | 2026-05-30 | Clean. Uses `api` wrapper, endpoints (`/workspace-invitations/{token}` GET + `/accept` POST) verified. |
| `pages/AcceptProjetInvitation.vue` | ✅ | 2026-05-30 | **Bug fixed:** used raw `axios` with `/api/...` prefix — switched to project's `api` wrapper for Bearer-token interceptor consistency. Endpoints (`/invitations/projet/{token}/check` + `/accept`) verified. |
| `pages/Auth/ForgotPassword.vue` + `ResetPassword.vue` | ⚠️ | 2026-05-30 | **Flagged not fixed:** both pages call `POST /auth/forgot-password` and `POST /auth/reset-password` which **do not exist as routes**. `config/fortify.php` has all features commented out (including `Features::resetPasswords()`), so the entire password-reset flow is broken end-to-end. Fixing requires enabling the Fortify feature + mail configuration + a working SMTP backend — infrastructure work outside this audit's scope. |
| `components/common/LanguageSwitcher.vue` | ✅ | 2026-05-30 | Clean. Calls `authStore.setLanguage()` which handles backend persistence + i18n locale switch. |

### Profile / Settings

| Page | Status | Date | Findings |
|---|---|---|---|
| `pages/Others/UserProfile.vue` (routed `/profile`) | ✅ | 2026-05-30 | **Bug fixed:** redirect after account deletion went to `/login` — only `/signin` exists in the router. Tabs + child components otherwise clean. |
| `components/profile/ChangePasswordModal.vue` | ✅ | 2026-05-30 | **Critical bug fixed:** called `axios.put('/api/profile/password', this.form)` — `axios` not imported (worked only via `window.axios` global from bootstrap.js) AND the route didn't exist. Switched to `api.post('/users/change-password', this.form)` matching the real `POST /users/change-password` route. |
| `components/profile/UserProfile.vue` | 🗑 | 2026-05-30 | **Deleted:** 261-line orphan (zero imports), broken internally (old AdminLTE/Bootstrap, wrong `/api/profile` endpoint, markup placed outside `<template>` wrapper). `pages/Others/UserProfile.vue` is the actually-used profile page. |
| `composables/useUsers.js` | ✅ | 2026-05-30 | **Bugs fixed:** 4 functions referenced `axios` without importing it (relied on `window.axios` from bootstrap.js — bypasses the `api` wrapper's Bearer-token interceptor + 401 refresh logic). Switched all to `api`. Removed duplicate `changePassword` key (later one shadowed the store one). Deleted dead `exportProfileData` (`GET /users/export` doesn't exist, no callers). |
| `pages/NotificationPreferences.vue` | ✅ | 2026-05-30 | Clean. Uses `api`, endpoints (`GET/PUT /notification-preferences`) verified, `useWebPush` integration wired correctly. |
