...
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| View project | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| View ALL projects in workspace (`projets.view_all`) | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| Edit project | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| Delete project | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
| Manage project members | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| Manage team integration — toggle use_teams, link/unlink teams (`projets.manage_teams`) | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
...
| 2026-06-28 | Phase 2 (Step 2.2) | `projets.manage_teams` added to `Permission.php` (const + `all()` + `forRole('owner')` via array_diff + `forRole('manager')` explicit). Mirror in `Permission.js`. `canManageTeams` computed in `useProjetPermissions.js`. `can_manage_teams` in `ProjetResource` user_permissions. `ProjetPolicy::manageTeams()` added. Seeded via `RolePermissionSeeder`. owner/manager ✅, cadre/below ❌. |
| 2026-06-28 | Phase: Visibility refactor (Step 1.1) | `projets.view_all` added to `Permission.php` (const + `all()` + `forRole('owner')` via array_diff + `forRole('manager')` explicit). Mirror in `Permission.js`. `canViewAll` computed in `useProjetPermissions.js`. Matrix project-level row added. Replaces the `visibility` column–based access control: manager+ can see all workspace projects without being a member; cadre/collaborateur/stagiaire/observateur see only their joined projects. |
